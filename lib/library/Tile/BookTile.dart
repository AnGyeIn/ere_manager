                    ...
                    widget.isAdmin
                      ...
                      onPressed: () {
                        if (widget.book.isAvailable)
                          reference.child('library').child('Book').child(
                              widget.book.id).remove().whenComplete(() {
                          widget.refresh();
                          EREToast(str.lang == '한국어' ? '도서가 삭제되었습니다.' : 'The book is removed.', context, false);
                          });
                        else
                          EREToast(str.lang == '한국어'
                    ...
                    FlatButton(
                      ...
                      onPressed: () async {
                        ...
                        if (readIsbn == widget.book.isbn) {
                          ...
                          final borrowTransaction = await reference.child('library').child('Book').child(widget.book.id).runTransaction((mutableData) async {
                          ...
                          final addPersonalRentalTransaction = await reference.child('Student').child(widget.userID).child('rental').child(id).runTransaction((mutableData) async {
                            mutableData.value = jsonEncode({
                              'id': id,
                              'bookID': widget.book.id,
                              'borrowerID': widget.userID,
                          ...
                          if (borrowTransaction.committed && addRentalTransaction.committed && addPersonalRentalTransaction.committed) {
                            EREToast(str.lang == '한국어'
                                ? '대출 처리가 완료되었습니다.'
                                : 'Succeeded to borrow the book.', context,
                                false);
